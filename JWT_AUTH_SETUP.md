# Staff Portal - JWT Authentication Setup

## Overview

The staff portal now implements the proper JWT authentication flow:
...
1. User selects team (ops/support/finance/marketing)
   ↓
2. Enters email/password
   ↓
3. Firebase sign-in
   ↓
4. Get Firebase ID token (ONCE)
   ↓
5. POST /api/auth/verify with Firebase token
   Authorization: Bearer <firebase_id_token>
   ↓
6. Backend verifies and returns backend JWT
   ↓
7. Store backend JWT in localStorage
   ↓
8. All API calls use backend JWT
...
console.log('JWT:', localStorage.getItem('staffToken'));
...
- Call `auth.currentUser.getIdToken()` in API request methods
...
**Solution:**
- Verify Firebase environment variables are set in `.env`
- Check that all required Firebase config values are present
- Restart dev server after updating `.env`

## Files Modified

- ✅ `src/services/firebase-auth.ts` - Proper token exchange
- ✅ `src/services/staff.service.ts` - JWT-only API client
...
- [Vendor Portal Setup](../vendor/JWT_AUTH_SETUP.md)
