    ...
    <div>
      ...
      {!loading && payouts.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-4" style={{ boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          <div className="grid px-4 py-2.5 border-b border-gray-100 text-xs font-bold text-gray-400 uppercase tracking-wide" style={{ gridTemplateColumns: "1fr 120px 120px 160px", gap: "12px" }}>
            <span>Vendor</span>
            <span>Amount</span>
            <span>Status</span>
          ...
          {payouts.map(p => (
            <div key={p.id} className="grid px-4 py-3 border-b border-gray-50 last:border-0 items-center" style={{ gridTemplateColumns: "1fr 120px 120px 160px", gap: "12px" }}>
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-semibold w-fit truncate ${statusColor[p.status] || "bg-gray-100 text-gray-600"}`}>{p.status}</span>
              <div className="flex justify-end">
                {p.status === "issue" && !raised.includes(p.id) && (
                  <button onClick={() => setTicketModal({ id: p.id, vendor: p.vendor })}
                    className="flex items-center gap-1 px-2.5 py-1.5 border border-orange-200 text-orange-600 text-xs font-bold rounded-lg hover:bg-orange-50 transition whitespace-nowrap">
                    <AlertTriangle size={11} /> Raise Ticket
                  </button>
                )}
